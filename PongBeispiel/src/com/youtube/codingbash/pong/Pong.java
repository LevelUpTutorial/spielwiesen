...
public class Pong extends Applet implements Runnable, KeyListener, MouseListener {
	...
	// TODO - add objects and variables here as needed to model a blackjack game
	...
	public void run() {
		...
		// TODO - add any actions that need to happen right at the beginning of the game
	...
	public void checkCollision() {
		if (paddle1.getPad().intersects(puck.getCircle()) || paddle2.getPad().intersects(puck.getCircle())) {
			puck.setVelocity(new Point(-puck.getVelocity().x, puck.getVelocity().y));
		}
		if (paddle1.getPad().intersects(puck.getCircle())) {
			...
			} else if (puck.getCircle().y > paddle1.getPad().y + paddle1.getPad().getHeight() - paddle1.getPad().getHeight() / 3) {
				puck.setVelocity(new Point(puck.getVelocity().x, 10));
			...
			if (puck.getCircle().y < paddle2.getPad().y + paddle2.getPad().getHeight() / 3) {
				puck.setVelocity(new Point(puck.getVelocity().x, -10));
			} else if (puck.getCircle().y > paddle2.getPad().y + paddle2.getPad().getHeight() - paddle2.getPad().getHeight() / 3) {
				puck.setVelocity(new Point(puck.getVelocity().x, 10));
			} else if (puck.getVelocity().y < 0) {
				puck.setVelocity(new Point(puck.getVelocity().x, -5));
			} else if (puck.getVelocity().y > 0) {
